...
#### 방법 C. Git Bash + Vim 에디터 활용 시
터미널에서 내장 Vim 에디터로 충돌 파일을 열어 수정합니다.
```bash
vim hello.txt
```
1. **편집 모드 전환**: 키보드 영문 **`i`** 키를 누릅니다. (화면 아래에 `-- INSERT --` 또는 `-- 끼워넣기 --` 문구가 뜨면 편집 가능 상태가 됩니다.)
2. **직접 편집**: 키보드 방향키로 이동하며 충돌 기호들(`<<<<<<<`, `=======`, `>>>>>>>`)을 지우고 내용을 정돈합니다.
3. **저장 및 탈출**: **`Esc`** 키를 누른 뒤, **`:wq`**를 입력하고 **`Enter`**를 쳐서 저장하고 빠져나옵니다.
...
1. **팀원 B**는 충돌이 발생한 파일을 에디터로 엽니다.
   ...
   * **Git Bash + 메모장 사용 시**: `notepad README.md` 명령어로 메모장을 실행해 특수 기호들(`<<<<<<<`, `=======`, `>>>>>>>`)을 직접 삭제하고, A와 B의 한 줄 인사가 모두 남도록 정리합니다.
   * **Git Bash + Vim 사용 시**: `vim README.md` 명령어로 파일을 연 뒤, `i`를 눌러 입력 모드로 들어가 기호들을 지우고 정리한 다음, `Esc` 키를 누르고 `:wq`를 친 뒤 `Enter`로 저장하고 나옵니다.
