...
3. [협업 필수 명령어 & 흐름 (Do's)](#협업-필수-명령어--흐름-dos)
4. [방어적 협업의 핵심: 로컬 브랜치 최신화 흐름](#방어적-협업의-핵심-로컬-브랜치-최신화-흐름)
5. [협업 시 절대 하지 말아야 할 것 (Don'ts)](#협업-시-절대-하지-말아야-할-것-donts)
6. [실전! 협업 에러 시나리오 3선 및 대처법](#실전-협업-에러-시나리오-3선-및-대처법)
...
2. **`git switch <브랜치명>`**: 작업할 방으로 들어가기
...
> [!NOTE]
> **`git checkout`과의 관계**: 인터넷 검색이나 오래된 강의에서 `git checkout`이라는 명령어를 자주 보게 됩니다. `checkout`은 과거에 **브랜치 전환 + 파일 복원**을 혼자서 전부 처리하던 명령어인데, 역할이 너무 많아 헷갈린다는 이유로 Git 2.23(2019) 버전부터 **`git switch`**(브랜치 전환)와 **`git restore`**(파일 복원)로 분리되었습니다. 이 가이드에서는 최신 권장 명령어인 `switch`만 사용합니다.
