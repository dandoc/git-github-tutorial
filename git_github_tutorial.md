...
2. [⚙️ Git 최초 설정: 사용자 등록 (이름과 이메일)](#%EF%B8%8F-git-%EC%B5%9C%EC%B4%88-%EC%84%A4%EC%A0%95-%EC%82%AC%EC%9A%A9%EC%9E%90-%EB%93%B1%EB%A1%9D-%EC%9D%B4%EB%A6%84%EA%B3%BC-%EC%9D%B4%EB%A9%94%EC%9D%BC)
3. [📁 GitHub 레포지토리 시작하기](#-github-레포지토리-시작하기)
4. [💻 기초 명령어 및 개인 작업 흐름 (Do's)](#-기초-명령어-및-개인-작업-흐름-dos)
5. [🛡️ .gitignore: 올리면 안 되는 파일 관리하기](#%EF%B8%8F-gitignore-올리면-안-되는-파일-관리하기)
6. [🏷️ GitHub Issue와 커밋 연결하기 (실무 필수)](#%EF%B8%8F-github-issue와-커밋-연결하기-실무-필수)
7. [🌊 방어적 협업의 핵심: 로컬 브랜치 최신화 흐름](#-방어적-협업의-핵심-로컬-브랜치-최신화-흐름)
8. [❌ 협업 시 절대 하지 말아야 할 것 (Don'ts)](#-협업-시-절대-하지-말아야-할-것-donts)
9. [🚨 실전! 협업 에러 시나리오 5선 및 대처법](#-실전-협업-에러-시나리오-5선-및-대처법)
...
## ⚙️ Git 최초 설정: 사용자 등록 (이름과 이메일)

Git을 설치한 후 **가장 먼저 해야 하는 필수 설정**입니다. Git은 버전을 기록(커밋)할 때 "누가 이 버전을 작성했는지" 표시하기 위해 작성자 정보(이름과 이메일)를 필요로 합니다. 

이 설정을 건너뛰면 나중에 커밋(`git commit`)할 때 `Author identity unknown` 에러가 발생하며 진행이 막히게 됩니다.

### 1단계: 사용자 이름과 이메일 등록하기
터미널(Git Bash, 터미널 등)을 열고 아래 명령어를 입력합니다. 
`--global` 옵션을 붙이면 내 컴퓨터의 모든 Git 저장소에 이 정보가 기본값으로 자동 설정됩니다.

```bash
# Git에 사용할 이름(또는 GitHub 사용자명) 설정
...
```
> [!IMPORTANT]
> **이메일 작성 팁**: 이메일 주소는 **GitHub에 가입할 때 사용한 이메일 주소**와 동일하게 설정해야, 변경 사항이 GitHub에 업로드되었을 때 본인의 잔디(Contribution graph)가 정상적으로 심어집니다!

### 2단계: 설정 정보 확인하기
설정이 잘 들어갔는지 확인하려면 아래 명령어를 입력합니다.
...
### 🚨 시나리오 5: 실수로 `-m` 없이 `git commit`을 쳐서 이상한 화면(Vim)에 갇혔을 때
> **상황**: 커밋 메시지를 깜빡하고 `git commit`만 입력하고 엔터를 쳤더니, 터미널이 검은색 화면으로 변하고 아래에 `~` 기호들이 가득 찬 무서운 화면이 나타났습니다. 키보드를 쳐도 글씨가 써지지 않고 나갈 수도 없습니다.

#### 💡 해결법: Vim 에디터 탈출 공식 외우기
이 화면은 Git의 기본 텍스트 에디터인 **Vim(빔)**입니다. 아래 순서대로 정확히 키보드를 누르면 빠져나갈 수 있습니다.

1. **Vim 상태 초기화**: 키보드 좌측 상단의 **`Esc`** 키를 2~3번 누릅니다. (입력 모드 해제)
2. **명령어 입력**: **`:`**(콜론) 키를 누른 뒤 영문 **`q`**와 **`!`**(느낌표)를 연속으로 칩니다. (저장하지 않고 탈출)
   * 화면 맨 아래 왼쪽 구석에 **`:q!`**라고 적힌 것을 확인합니다.
3. **엔터(Enter) 입력**: **`Enter`**를 치면 원래 터미널 화면으로 안전하게 돌아옵니다.

> [!TIP]
> **Vim 화면에서 커밋 메시지를 적고 저장해서 나가고 싶다면?**
> 1. **`Esc`** 키를 몇 번 누릅니다.
> 2. 영어 알파벳 **`i`** 키를 누릅니다. (화면 아래에 `-- INSERT --` 또는 `-- 끼워넣기 --`가 나타납니다.)
> 3. 화면 맨 첫 번째 줄에 원하는 커밋 메시지를 입력합니다 (예: `feat: 로그인 기능 추가`).
> 4. 입력을 마쳤으면 **`Esc`** 키를 누릅니다.
> 5. **`:`** 키를 누른 뒤 영문 **`wq`**를 칩니다. (Write and Quit, 저장 후 종료)
>    * 화면 아래에 **`:wq`**가 적힌 것을 확인하고 **`Enter`**를 칩니다.
